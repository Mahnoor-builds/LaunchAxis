...
import { faTruckFast, faHeadset, faShieldHalved, faArrowRightArrowLeft, faArrowRight, faStar } from '@fortawesome/free-solid-svg-icons';
...
const ShopHome = ({ branding, products = [], addToCart, siteConfig = {}, cartCount, openCart }) => {
  ...
  // --- NEW FILTERING LOGIC ---
  // 1. Get ONLY explicitly featured products (Max 6)
  const featuredProducts = products.filter(p => p.isFeatured).slice(0, 6);
  
  // 2. Get 4 general products so the homepage is never empty
  ...
  // --- ROUTING LOGIC ---
  // Takes the user to the catalog and remembers which category they clicked!
  const goToCatalog = (categoryName = 'All') => {
      localStorage.setItem('launchAxisStoreCategory', categoryName);
      
      // Use React Router to officially switch the page to the Catalog
      navigate('/catalog'); 
      window.scrollTo(0, 0);
  };
  ...
  return (
    <ShopLayout branding={branding} cartCount={cartCount} siteConfig={siteConfig} openCart={openCart}>
      ...
      <div style={{ background: '#f8fafc', color: '#0f172a', fontFamily: fontFamily }}>
        ...
        {/* === 2. DYNAMIC CIRCULAR CATEGORY CARDS (HIDDEN IF EMPTY) === */}
        {categories.length > 0 && (
            <section id="category-circles" style={{ paddingTop: '80px', paddingBottom: '30px', maxWidth: '1400px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', justifyContent: 'center', padding: '0 20px' }}>
                  ...
                  >
                      <div style={{ 
                          ...
                          border: `4px solid #e2e8f0`, boxShadow: '0 10px 20px rgba(0,0,0,0.08)', transition: 'all 0.3s',
                      ...
                      <span style={{ fontSize: '14px', fontWeight: '700', color: '#0f172a' }}>All Products</span>
                  </div>
                  ...
                  {categories.map(cat => (
                      ...
                      >
                          ...
                          <span style={{ fontSize: '14px', fontWeight: '700', color: '#0f172a' }}>{cat.label}</span>
                      </div>
                  ))}
              </div>
          </section>
        ...
        {/* === 3. FEATURED PRODUCTS (ONLY SHOWS STARRED ITEMS, MAX 6) === */}
        {featuredProducts.length > 0 && (
          <section id="featured" style={{ padding: '40px 5% 40px 5%', maxWidth: '1400px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '2rem', fontWeight: '900', margin: '0 0 30px 0', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <FontAwesomeIcon icon={faStar} style={{ color: '#eab308' }} /> Featured Selections
            </h2>
        ...
        {/* === 4. CATALOG PREVIEW (ALWAYS SHOWS 4 ITEMS SO PAGE IS NEVER EMPTY) === */}
        {catalogPreviewProducts.length > 0 && (
          <section style={{ padding: '40px 5% 80px 5%', maxWidth: '1400px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '2rem', fontWeight: '900', margin: '0 0 30px 0', color: '#0f172a' }}>
                Latest Arrivals
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '40px' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '60px' }}>
                <button 
                  ...
                  style={{
                      ...
                      fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '10px', transition: 'background 0.2s'
                  }}
        ...
        {showAbout && (
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '80px', position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ flex: '1 1 400px', height: '500px', background: '#1e293b', borderRadius: '24px', overflow: 'hidden' }}>
...
// Extracted Product Card to keep code clean since we use it in two different sections now!
